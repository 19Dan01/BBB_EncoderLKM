 ...
 * @date    24 May 2016
 * @version 0.1
 * @brief  first Test of a LKM to communicate with BBB onbord qaudradure encoder 
*/

MODULE_LICENSE("GPL");              ///< The license type -- this affects runtime behavior
MODULE_AUTHOR("Daniel Bauernfeind");      ///< The author -- visible when you use modinfo
MODULE_DESCRIPTION("A Linux driver for the BBB eqep");  ///< The description -- see modinfo
...
struct EQEP_REGS {
	...
	REG16	q_capctl;		/*	0x20	capture control */
...
static ssize_t eqep_get_velocity(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	/* read time for Velocity measurement */
		velocity = ((readl(eqep->mmio_base + QCPRD))*((2*(readl(eqep->mmio_base + QDF)))-1));
			}
 ...
 */
static int __init EQEP-LKM_init(void){
   printk(KERN_INFO "EBB: Hello from the BBB EQEP LKM!\n");
   return 0;
}
 ...
 */
static void __exit EQEP-LKM_exit(void){
   printk(KERN_INFO "Goodbye from the BBB EQEP-LKM!\n",);
}
 ...
 *  listed above)
 */
module_init(EQEP-LKM_init);
module_exit(EQEP-LKM_exit);
